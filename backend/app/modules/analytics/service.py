    ...
    *,
    ...
    jobs_query = (
        select(
            ...
            func.sum(case((DeliveryJob.status == DeliveryJobStatus.FAILED.value, 1), else_=0)),
            func.sum(case((DeliveryJob.status == DeliveryJobStatus.RETRYING.value, 1), else_=0)),
            func.sum(case((DeliveryJob.status == DeliveryJobStatus.DEAD_LETTER.value, 1), else_=0)),
        )
        .select_from(DeliveryJob)
        .where(DeliveryJob.organization_id == organization_id, DeliveryJob.deleted_at.is_(None))
    )
    if environment:
        jobs_query = jobs_query.join(Event, Event.id == DeliveryJob.event_id).where(Event.environment == environment)
    jobs_query = _apply_date_range(jobs_query, DeliveryJob.queued_at, start_date, end_date)

    total_deliveries, success_count, failed_count, retrying_count, dead_letter_count = (
        await db.execute(jobs_query)
    ).one()
...
async def get_deliveries_over_time(
    ...
    *,
    ...
    if environment:
        query = query.join(Event, Event.id == DeliveryJob.event_id).where(Event.environment == environment)
    query = _apply_date_range(query, DeliveryJob.queued_at, start_date, end_date)
