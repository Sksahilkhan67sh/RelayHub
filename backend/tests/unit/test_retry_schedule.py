...
def test_default_max_attempts_matches_spec():
    # spec: immediate + 10s,30s,1m,5m,15m,30m,1h = 8 total attempts
    assert DEFAULT_MAX_ATTEMPTS == 8
    assert DEFAULT_RETRY_DELAYS_SECONDS == [10, 30, 60, 300, 900, 1800, 3600]


@pytest.mark.parametrize(
    "attempt_number,expected_base_delay",
    [(1, 10), (2, 30), (3, 60), (4, 300), (5, 900), (6, 1800), (7, 3600)],
)
...
def test_no_more_retries_after_max_attempts():
    delay = compute_next_retry_delay(attempt_number=8)  # 8th attempt just failed, max is 8
...
def test_endpoint_override_larger_than_schedule_reuses_longest_interval():
    ...
    assert delay is not None
    seconds = delay.total_seconds()
    assert 3600 * 0.8 <= seconds <= 3600 * 1.2
